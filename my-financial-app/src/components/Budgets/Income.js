...
const Income = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch('http://localhost:8000/add-income', {
                    method: 'POST',
    ...
    return (
        <div className="container mt-4" style={{width: '100%'}}>
            <div className="d-flex align-items-center mb-4">
                <Link to={'/'}>
                <i className="bi bi-arrow-left-square" style={{fontSize: '2rem', cursor: 'pointer', color: 'black'}}></i>
                </Link>
            </div>
            <form onSubmit={handleSubmit} className="card card-body bg-light mt-lg-5">
                <h3 className="mb-3">Add New Income</h3>
                <div className="mb-3">
